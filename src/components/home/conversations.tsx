		...
		<>
			<div className={`flex gap-2 items-center p-3 hover:bg-chat-hover cursor-pointer `}>
				<Avatar className='border border-gray-900 overflow-visible relative'>
					{conversation.isOnline && ( // if online the place  green dot
						<div className='absolute top-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-foreground' />
					)}
					{
						!conversation.isOnline &&(
							<div className='absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-foreground' />
						)
					}
