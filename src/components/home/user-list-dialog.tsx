 ...
 const generateUploadUrl= useMutation(api.conversations.generateUploadUrl);
   ...
   const handleCreateConversation=async()=>{
	...
	try{
		...
		let conversationId;
		if(!isGroup){  // if one to one conversation
           conversationId= await createConversation({
			participants :[...selectedUsers,me?._id],
			isGroup:false,
		  });
		}
		else  // if it is a group, we will create it!
		{
           const posturl=await generateUploadUrl();// short lived url
		   ...
		   // below we recive storage id from uloaded image!
		...
		await createConversation({
			...
			admin:me?._id!,
			groupName,
			groupImage:storageId,  // we send this storage id to backend
		});

		}

		dialogCloseref.current?.click(),  // after making group we return original dialog box back
        setSelectedUsers([]);
	    setGroupName("");
		...
		<Dialog>
			...
			<DialogContent>
				...
				{selectedUsers.length > 1 && (
					<>  {/*Above format && will execute if first condition is true!*/}
						<Input
